...
TString Lepton = "EPlusJets/";

//muon variables
const int N = 8;
TString Variable;
TString Variables[N] = {"all_jet_pT_", "all_jet_phi_", "all_jet_eta_", "jet1_pT_", "jet2_pT_", "jet3_pT_", "jet4_pT_", "jet5_pT_"};
double MinXs[N] = {0, -3.4, -2.6,0,0,0,0,0};
double MaxXs[N] = {500, 3.4, 2.6,500,500,500,500,500};
TString XTitles[N] = {"p_{T} (Jets)", "#Phi (Jets)", "#eta (Jets)","p_{T} (Jet 1)","p_{T} (Jet 2)","p_{T} (Jet 3)","p_{T} (Jet 4)","p_{T} (Jet 5)"};
...
data = getSample("SingleElectron", 1);
...
THStack *hs = new THStack("hs","test");
  ...
  TString plotName("plots/"+Lepton+Obj);
...
TH1D* getSample(TString sample, double weight){
	...
	TH1D* plot = (TH1D*) file->Get("TTbarPlusMetAnalysis/"+Lepton+Isolation+Obj+Variable+Nbtags);
	TH1D* plot2 = (TH1D*) file->Get("TTbarPlusMetAnalysis/"+Lepton+Isolation+Obj+Variable+"3btags");
	TH1D* plot3 = (TH1D*) file->Get("TTbarPlusMetAnalysis/"+Lepton+Isolation+Obj+Variable+"4orMoreBtags");
